...
a1 = double([ones(1,m); X']);

z2 = Theta1*a1;

a2 = double([ones(1,m); sigmoid(z2)]);

z3 = Theta2*a2;

a3 = double([ones(1,m); sigmoid(z3)]);

z4 = Theta3*a3;

a4 = double(sigmoid(z4));

reg_term = lambda/(2*m)*(sum(sum(Theta1(:,2:end) .^ 2)) + sum(sum(Theta2(:,2:end) .^ 2)) + sum(sum(Theta3(:,2:end) .^ 2)));

J = (-1/m * sum(sum((y_as_matrix .* log(a4)) + ((1-y_as_matrix) .* log(1 - a4))))) + reg_term;

% -------------------------------------------------------------

error4 = a4 - y_as_matrix;

error3 = (Theta3(:,2:end)' * error4) .* sigmoidGradient(z3);

error2 = (Theta2(:,2:end)' * error3) .* sigmoidGradient(z2);

Theta3_grad = (error4*a3') ./ m + lambda/m*[zeros(size(Theta3,1),1) Theta3(:,2:end)];
